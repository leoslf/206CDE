...
def get(name, code=None, writer=None, **kwargs):
    try:
        ...
        raise BarcodeNotFoundError('The barcode {0!r} you requested is not '
                                   'known.'.format(name))
    if code is not None:
        return barcode(code, writer, **kwargs)
    else:
        return barcode
...
def generate(name, code, writer=None, output=None, writer_options=None, **kwargs):
    options = writer_options or {}
    barcode = get_barcode(name, code, writer, **kwargs)
